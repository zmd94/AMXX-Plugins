...
public plugin_init() 
{
	register_plugin("Weapons War", "6.7", "zmd94")
	...
	g_WarMenu = menu_create("\yWeapons War \rv6.6", "war_handler")
...
public cs_war_WeaponsDeploy(iWeapon)
{
	if(g_bInWar && get_pcvar_num(g_iWeaponDrop))
	{
		...
		{
			set_pdata_float(iWeapon, m_flNextPrimaryAttack, 39.0, XO_WEAPONS)
			set_pdata_float(iWeapon, m_flNextSecondaryAttack, 39.0, XO_WEAPONS)
		}
	}
}  
